...
class Controller:
    ...
    def start_app(cls):
        ...
        menu_dict = {1: Controller.user_add_card,
                     ...
                     3: Controller.card_manager.view_all_cards,
                     4: Controller.user_view_card,
                     5: Controller.user_backup_data,
                     6: Controller.end_app}
        while True:
            try:
                ...
                print(f'{e} Please enter integer 1 to 6!\n')
        ...
        try:
            ...
            for card in cls.card_manager.cards:
                if card_name == card.card_name:
                    print(f'Card name {card_name} already exists!\n')
                    return False
            ...
            new_card = None
            date_format = '%Y-%m-%d'
            if issubclass(CardManager.card_type_dict[card_type_key], Expirable):
                issue_date = input(' Enter issue date(YYYY-MM-DD): ')
                issue_date = datetime.datetime.strptime(issue_date, date_format)
                expiry_date = input(' Enter expiry date(YYYY-MM-DD): ')
                expiry_date = datetime.datetime.strptime(expiry_date, date_format)
            ...
            if card_type_key == 1:
                ...
                new_card = CardManager.create_reward_card(card_name, card_holder,
    ...
    def user_remove_card(cls):
        card_name = int(input('Enter card name of the card to be removed: '))
